...
# A receipt that MATCHES this branch and commit but whose status says nothing.
# Before the counter-model review of this change, anything that was not exactly
# `skipped` was accepted as a completed review - so a receipt with a garbage
# status, or none at all, satisfied the gate while recording neither a review nor
# an allowed skip. That is #1171's own defect one level in.
write_receipt "$(git rev-parse HEAD)" garbage
