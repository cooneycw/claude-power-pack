...
# A skip reason that is shaped like one of grep's own options. Without `--`,
# `grep -qxF "-ecodex-absent"` reads it as `-e codex-absent` and MATCHES, so a
# reason absent from the committed set passed the allowlist. An allowlist that
# can be addressed with its own matcher's flags is not an allowlist.
write_receipt "$(git rev-parse HEAD)" skipped -ecodex-absent
