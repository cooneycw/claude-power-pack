...
# A FULL checkout - not shallow - whose receipt names a well-formed object name that
# exists nowhere. `merge-base --is-ancestor` exits 128 here exactly as it does in a
# shallow clone, and that is the point: without the shallow check the two are
# indistinguishable and this fabricated receipt would pass as `undecidable`.
write_receipt 0123456789abcdef0123456789abcdef01234567 ran
