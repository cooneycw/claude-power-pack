...
# A skip naming a reason the committed set does NOT contain. `explicit-opt-out`
# was REMOVED deliberately; accepting it here would re-open the hole under a
# name nobody re-added to SKIP_REASONS.
write_receipt "$(git rev-parse HEAD)" skipped explicit-opt-out
