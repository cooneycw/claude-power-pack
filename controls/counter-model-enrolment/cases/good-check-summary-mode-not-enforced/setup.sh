...
# /flow:check Step 5 advisory mode, in an ENROLLED repo whose receipt state is
# as bad as it gets. It must NOT be enforced: check-summary produces no PR, so
# it has nothing to enrol and no way to satisfy the check.
write_receipt deadbeef skipped explicit-opt-out
printf -- "--check-summary" > args.txt
