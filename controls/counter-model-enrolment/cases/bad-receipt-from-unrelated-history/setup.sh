...
# A real, well-formed receipt whose head belongs to a DIFFERENT history. A
# review that happened somewhere else is not a review of this branch.
git checkout -q --orphan other; git commit -qm unrelated --allow-empty
OTHER=$(git rev-parse HEAD); git checkout -q master
write_receipt "$OTHER" ran
