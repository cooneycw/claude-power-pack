...
story B1, ratified hybrid SoT): evolve the flat custom-prompt surface
(scripts/codex-prompt-sync.py, issue #446, now DEPRECATED pending the issue
#556 cutover) into real Codex skills. ADR 0001 section 5 fixes the source of
...
orphan-deleted. Mirrors the plugin-sync.sh / codex-prompt-sync.py idioms:
...
# Same packaged families as plugin-sync.sh and codex-prompt-sync.py (ADR 0001
# target design), minus the `codex` family: those commands orchestrate the
# Codex CLI itself, so shipping them INTO Codex as skills would be circular.
FAMILIES = [
...
# Per-family source basenames excluded from generation (mirrors
# codex-prompt-sync.py):
#   cpp: the legacy symlink installer retired in Phase B4.
#   self-improvement/memory.md: Codex ships the hand-curated equivalent
#     codex/prompts/cpp-memory.md (issue #433); a generated variant would
