#!/usr/bin/env bash
#: HOST-SURFACE: none under $HOME
#  writes PG_HBA, a system path, not a home surface. Reaches outside $HOME via apt, apt-get and curl, so it is
#  NOT observation-certified.
