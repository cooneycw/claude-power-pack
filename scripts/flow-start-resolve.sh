...
#     CROSS_REPO=0|1        1 = EnterWorktree cannot reach the target repo;
#                           enter with `cd`, clean up via the git fallback
...
# 4. Fresh start. Native lane (EnterWorktree) creates the worktree itself;
#    the cross-repo lane (#578) cannot use EnterWorktree, so create here.
if [ -z "$LANE" ]; then
  WT_PATH="$TARGET_REPO/.claude/worktrees/$BRANCH"
