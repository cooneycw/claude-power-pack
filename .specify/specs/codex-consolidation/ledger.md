...
| cxpp#247 - Skipped security gate reports ok, not warn | open defect | unassigned | CPP #1027 shipped per-verdict exit codes (PR #1057, merged 2026-09-19) | `already-covered` - **verify against `1fb0589`** ("give every gate verdict its own exit code", PR #1057, merged 2026-09-19), not against #1067's `194f7305`, which predates the fix. **Corrected 2026-09-22:** this row previously named `5ceb966a`, which is PR #1077 (the shared-stash guard) and is unrelated to cpp#1027. The PR number was right and the SHA was wrong, so a reader following the SHA would have verified against the wrong change |
...
| Entry | Status | Owner | Destination | Disposition |
|---|---|---|---|---|
| cxpp#276 - project-next recommends continue_work on a CLOSED issue | **live defect** | unassigned | **cpp#1035** (re-pointed 2026-09-22; was #1069) | `move` - travels with project-next ownership. CPP #1035 records the same family of defect; reuse both scopes and their tests, do not file duplicates. **Destination corrected 2026-09-22:** cpp#1069 CLOSED without carrying the obligation - the defect is still live at `lib/project_next/rank.py:232-238`, where `continue_work` is returned with `evidence=("worktree:...", "dirty:true")` and no issue-state guard, and no test covers it. A `move` into a closed destination that never received the item is a defect in THIS ROW, not a new decision, so re-pointing it executes the owner's ruling rather than overriding it. Evidence recorded on cpp#1035 BEFORE cxpp#276 was closed, so the obligation was never in flight |
