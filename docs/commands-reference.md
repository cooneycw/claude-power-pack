...
- Pre-implementation gate (issue #774): all three delegated drivers - `/codex:auto`, `/qwen:auto`, `/gemma:auto` - STOP at `Step 3/8: Approve` after reporting the plan and wait for approval before invoking the model CLI. Before #774 each printed a plan that read exactly like a checkpoint and then delegated in the same turn; because their Review step inspects a diff, that boundary is the only point at which anything can be caught before code exists. Found on a six-worker kyle orchestration wave where three workers described the boundary as a halt it did not have. `--yes` (alias `--auto-approve`) was the only bypass, and it was deliberately flag-only: no `eli5: auto-approve`-style trailer was honored, since a marker in an issue body or commit message is written by the filer rather than the invoker (the #775 hazard, never propagated here). Issue #784 then removed the flag too - see below. `tests/test_delegated_driver_gates.py` pins the gate, the ordering, the absence of any bypass, and the absence of a trailer bypass in all three drivers.
- Driver capability contract (issue #783): the gates above answer *should this work be done, and is this the right plan?*; nothing answered *can this driver do this work at all?*. Each of the four drivers now declares its capability on two axes - `scope` (general | implementation-only) and `web` (yes | no) - in `scripts/flow-driver-capability.sh`, stated as a "Capability contract" section in its own skill file, and checked as a Step 2/8 pre-flight before any prompt is built. The three delegated drivers are `implementation-only` (their execution fence makes the deliverable a source diff, so a RESEARCH ticket comes back diff-shaped because that is all the model can produce) and web-less (`codex --sandbox workspace-write` blocks network; `gemma-implementer` DENIES `webfetch`/`websearch` by name; the Qwen lane configures no retrieval tool), so a live-source ticket comes back confidently answered from training data. Both were observed on the `kyle-completion` wave (2026-09-05) on two different drivers in one evening, and both were caught only because the worker read its own fence and refused - discipline, not mechanism, the same gap #774 closed for halting. `/flow:wave` routes on it: `flow-wave-registry.sh register --driver <d>` records what a role is running and `list` renders `driver=gemma:auto[impl-only,no-web]`, so the orchestrator sees the mismatch when it ASSIGNS rather than when the worker refuses. Needs are DECLARED, never inferred from issue prose (#683's cry-wolf rule), and an unknown driver reports `unknown` and exits 0 rather than blocking a wave. Distinct from the #758 PRECONDITION split (`auto` needs a filed issue and a checkout, `exec` does not) - that is about whether the driver can start, this about what it can produce. `tests/test_driver_capability.py` derives the expected matrix from the driver documents, so a fence that disappears or a sandbox flag that changes turns the suite red rather than leaving the data quietly lying.
- No-bypass ELI5 gate (issue #775): `/flow:auto` Step 3 cannot be skipped at all. The `eli5: auto-approve` trailer channel is removed - it was read from the issue body or the HEAD commit message, neither of which the invoker writes, so a single merged commit carrying it disarmed the gate for every run branched from that tip. `--yes` / `--auto-approve` went with it: a flag typed at invocation approves a plan that does not exist yet. Both are still recognized, and refused out loud. `auto-granted` is gone from the Step 3 report vocabulary, since a producible field means a reachable bypass. The fix landed in the vendored `eli5-core` (canonical: cooneycw/eli5-gate) so it propagates rather than staying a CPP-local override, and `tests/test_eli5_gate_not_bypassable.py` pins it across every flow surface. Issue #784 then applied the same standard to the delegated drivers' Step 3/8 gate, so all four gates in the repo now hold to it.
- Failure detection across all six delegated surfaces (issue #798): every lane decided whether a run had failed by testing `$?` against zero, and in all six command documents that test could not observe the CLI's exit status - so a dead endpoint, a timeout, or an API error read as a completed task and `/*:auto` marched into review, quality gates, `/flow:finish` and `/flow:merge` on an empty diff. Three independent causes. `... | tee "$OUT"` makes `$?` the status of **tee** (`pipefail` set nowhere - grepped, zero hits across all three lanes), so the documented `exit 124 = timeout` branch was literally unreachable. In the three `auto` files the invocation and the check sat in SEPARATE fenced blocks with prose between them - separate shells, so `$?` reflected whatever ran last in the new shell rather than the run. And the Qwen CLI reports success on a failed run outright: verified 2026-09-07 against an unreachable endpoint (`@qwen-code/qwen-code` 0.15.10), `EXIT=0`, `subtype: "success"`, `is_error: false`, `num_turns: 1`, the only evidence of failure being `[API Error: ...]` inside the terminal `result` string - which is why no exit-code fix alone was sufficient and `scripts/delegated-run-check.sh` judges the PAYLOAD as well. All six documents now redirect rather than pipe and capture the status in the same block; the three `auto` lanes pass `--expect-tools` (a delegated implementation that used no tools wrote no code) and fail closed on an empty diff at Step 4 and again at Step 7, counted with `git status --porcelain` so a model that only ADDED files is not killed as having changed nothing. `/codex:auto`'s quality-gate fix loop carried the same `| tee` shape and is fixed too - a silently failed retry used to burn both attempts and then report the original gate failure as the diagnosis. Found while diagnosing why `/qwen:*` had been pointed at an offline MacBook; the endpoint was wrong long enough to matter and nothing surfaced it. `tests/test_delegated_lane_failure_detection.py` pins the six documents against every shape that broke - these are prompt documents, so the test is the mechanism rather than the discipline it replaces
