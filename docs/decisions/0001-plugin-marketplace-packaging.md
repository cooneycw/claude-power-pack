...
- **B1 - Scaffold + proof-of-concept.** Add `.claude-plugin/marketplace.json`
  and one family plugin (`flow`) as `plugins/flow/`. Prove
  `/plugin install flow@cpp` loads the commands alongside the existing
  installer. Retire nothing.
- **B2 - Migrate remaining families.** Package the other families as plugins,
  settling the single-source `commands/` + `skills/` layout shared with #446.
