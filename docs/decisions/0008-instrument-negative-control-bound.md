...
| 63 | `check-negative-controls.py` (CI `negative-controls` with `--strict`; `make negative-controls` with `--strict --allow-unavailable`) | per-gate `PASS\|BLIND\|INERT\|UNRESOLVED\|UNPROVEN\|UNSIGNALLED\|UNAVAILABLE`, then `negative-controls: ok - N of M ... discriminate` plus a named list of any UNEXAMINED controls, or a non-zero refusal. `UNAVAILABLE` (#1117) is the gate's own tool being absent - an environment fact, tolerated only under `--allow-unavailable` and never silently | CI at `.woodpecker.yml`, and `make verify`, both of which let work through on its verdict; read by other sessions and other repos without re-derivation (#964, #981) | G, X |
