...
|---|---|---|---|---|
| 1 | `flow-finish-gate.sh` + `lib.cicd run` / `resume` | `FLOW_FINISH_GATE: ok\|warn\|skipped\|fail`, and since #1027 the exit code that carries it (0/3/4/1); since #1147 the published gate set must be accounted for, and since #1155 the resolved plan must carry every gate the built-in plan of its name declares (`fail (gate dropped from the plan: ...)`, with `null` reported as not-applicable rather than as clean). Three registered controls: `controls/flow-finish-gate` (Makefile fallback, af348f8), `controls/flow-finish-gate-declared-gates` (runner lane accounting, af348f8) and `controls/flow-finish-gate-plan-reconciliation` (manifest-vs-declaration, e7c8708 - a separate registration because that anchor DETECTS the declared-gates control's known-bad input and would score it INERT) and `controls/flow-finish-gate-subsumption` (a not-run record cannot coexist with a pass, 529d470). THE SUBSUMPTION DERIVATION ITSELF IS NOT COMMITTED-CONTROL-COVERED and must not be re-registered as one: no control whose gate path reaches `lib.cicd` can run in the negative-controls image, measured inside it - a read-only workspace makes `uv` fail on the venv lock and the gate print `fail`, which scores a BAD case correct for the wrong reason; with a writable workspace and no network BOTH cases fail, so the GOOD one reds the battery; bare `python3` has no `pydantic`, which `lib/cicd/__init__.py` imports; `lib/cicd/steps.py` has module-level relative imports and will not load by file path; and the step's `depends_on` omits the one running `uv sync`, so the venv is not reliably present either. Filed as #1163, third instance after #987 and #1036. The derivation is pinned by `tests/test_runner.py::TestSubsumedGates`, which runs under `validate` where a venv exists | the Step-6 commit, push and PR; the Step-7 re-gate; the same gate in kyle and CxPP | G, X |
