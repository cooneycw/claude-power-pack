...
| 97 | `host-surface-check.py` (`controls/host-surface`) | `host-surface: ok` (exit 0) / `FAIL - N of M reachable script(s) declare no host surface` (exit 1) / `UNKNOWN - derived an empty population` (exit 2), the last covering a moved or missing command document rather than a compliant tree | `make verify`, through `make host-surface-check`, which lets work through on this gate's verdict and does not re-derive it; a consumer's drift gate reads the manifest it validates. **Registered** |
