...
| 17 | `flow-wave-lexicon.sh` | `FLOW_LEXICON` accept/reject of a transition token | the wave protocol acting on the message. Registered control `controls/flow-wave-lexicon` (#1189): a `GATE: HOLD` whose reason was written as prose beneath the token was ACCEPTED and stored as `(no reason given)`, so the hold stayed enforced while its argument was lost; it is now refused. Its anchor is HISTORICAL (c5b1c5e) and answers `ok` identically on the bad and good cases | X |
| 18 | `flow-wave-plan.py` | the startable set and path-contention index; since #1189 the ledger reader also reports how many rulings named a subject it cannot plan against, so a skipped entry stays distinguishable from a file that never held one | which issues an orchestrator hands out. Registered control `controls/flow-wave-plan` (#1189): one ruling whose subject is not an issue number used to make `int()` raise and the WHOLE ledger be rejected, so a hold stopped being enforced because of an entry that had nothing to do with it. Its anchor is HISTORICAL (250d56f) and misses the known-bad input by refusing the ledger - which the runner must score as a MISS and not as `unavailable`, since the pre-fix planner looked at a readable file and discarded it | X |
...
| 27 | `check-negative-fixture-preconditions.py` | `negative-fixture: ok - N wholesale PATH replacement(s) in M of K test file(s) are in functions that assert a precondition`, followed by its `not inspected:` scope line; a finding line naming the unasserted sites; and since #1180 `negative-fixture: UNKNOWN - ` when any test file could not be parsed. THE ROW'S QUOTED MESSAGE WAS STALE UNTIL #1180: it carried the pre-#933 wording `ok - every constructed absence ...`, which is the ANCHOR's line and not the gate's, under an annotation describing the overclaim #933 had already fixed - a census row reading true and being false, which is the defect class this ADR exists to bound. When UNKNOWN fires it is the SOLE verdict marker: findings from readable files still print, as labelled diagnostics, but are not scored, because a run holding an unexamined file cannot support a coverage claim and `detect_signal` is how the harness scores a successful detection. The trade is recorded at the decision site - a masked marker is recoverable, a refusal scored as a detection certifies a blind instrument as sighted. Registered control `controls/check-negative-fixture-preconditions` (anchor eae94b5), whose two `unknown-*` cases each declare `anchor_expect: clean` with a reason: the frozen anchor has no survey and no refusal branch at all (measured: zero occurrences of `unparseable`, `UNKNOWN` or `files_scanned`), so it answers `ok` over a population it never established | `make verify`; CI `validate` | G |
...
| 91 | `verify-coverage-check.py` Its Makefile reader moved to `lib/cicd/makefile_declaration.py` in #1162 and is imported rather than defined here; the control's verdicts were captured before and after the move and are byte-identical, which is the evidence that an extraction changed no instrument. `lib/cicd/makefile.py::parse_makefile` reads the SAME module and is deliberately NOT a row of its own: its only consumers are `generate_manifest`, a generator whose output a caller reviews, and `check_makefile`, reached by the advisory `lib.cicd check` - both re-derive, which is the bound's own exclusion. What is load-bearing is the reader underneath, and that is covered here and by `check-ci-coverage.py`; the #1162 extraction put `parse_makefile` behind two controls that already existed rather than needing a third, and the move was proved by capturing both controls' verdicts before and after and requiring them byte-identical. | `VERIFY_COVERAGE_TARGETS:` / `_EXAMINED:` / `_SCRIPTS:` / `_CENSUS:` on every run, then `UNACCOUNTED:` / `MISCLASSIFIED:` / `STALE:` / `UNDECLARED:` findings (exit 1) or `verify-coverage-check: ok - ...` (exit 0); `_CENSUS: unread` says the `not-a-checker` cross-check did not run, because unread must not read as empty | `make verify` - as a prerequisite, and again as the RECIPE that prints the closing 'what this run did NOT examine' report. Nothing downstream re-derives either: the report IS the consumer, and a report that under-names is indistinguishable from a repository with less to name. It is the only instrument that can see row 40's own failure mode - a sub-gate silently dropped from `verify`'s prerequisite list, which no member row can self-report - and the only one that asks whether a checker in `scripts/` is reachable from any gate at all (#1028). Controlled at `controls/verify-coverage` | G |
...
| 102 | `check-ci-coverage.py` (`controls/ci-coverage`) Since #1162 it IMPORTS that reader instead of loading `verify-coverage-check.py` by file path, so this gate's population no longer depends on a sibling gate's private layout. | `CI_COVERAGE_PREREQUISITES:` / `CI_COVERAGE_STEPS:` on every examined run, then `UNDECLARED:` / `MALFORMED:` / `ABSENT-STEP:` / `ORPHANED-CLAUSE:` findings (exit 1) or `check-ci-coverage: ok - N verify prerequisite(s) dispositioned against M pipeline step(s): R run in CI, E excluded` (exit 0); `check-ci-coverage: UNKNOWN - derived 0 prerequisites` / `0 steps` (exit 2) when a population came back empty, which an accounting gate must not print `ok` over | `make verify`, through `make ci-coverage-check`, and the CI `ci-coverage-check` step - both let work THROUGH on its green and neither re-derives whether a gate is dispositioned against CI. It is the only instrument that reads the `verify:` prerequisite list AGAINST `.woodpecker.yml`: #1146 found 27 gates in `verify` and 16 in the pipeline with nothing in the tree able to say so, and #1144 merged green over one of the eleven, leaving `main` red from the moment it landed. A declaration is a CLAIM, so both sides are read - `ci: runs <step>` naming a step the pipeline has not got reds by name. `ORPHANED-CLAUSE` names, by file and line, a `## ci:` written on its own line rather than on the directive: such a line reads as a declaration to a human and is invisible to a gate parsing only directives, which is this row's own subject one level in, and the `UNDECLARED` it would otherwise surface as is a false diagnosis. The gate is a MEMBER OF ITS OWN POPULATION and declares `ci: runs ci-coverage-check` on itself; a check auditing "does CI run what verify runs" that is itself absent from CI is the class it audits. Sibling of row 91 rather than a branch inside it: that verdict already stands for two populations and calls itself a tripwire, and this is a per-prerequisite enumeration over a third pair. Controlled at `controls/ci-coverage`, whose anchor is CONSTRUCTED: the declaration-COUNTING framing this issue itself used at `a9408bc` before the gate existed | G |
| 103 | `host-surface-observe.py` (`controls/host-surface-observe`) | `host-surface-observe: ok - N member(s) ran twice against a sandboxed HOME and wrote nothing they did not declare` (exit 0) / `UNDERSTATED - <script> wrote surfaces it did not declare` / `FALSE CLAIM - <script> declares ~/<surface> certified=observed, but this run did not observe it` / `UNCLASSIFIED - <script> is in the derived population with no observable/escapes decision; it was NOT run` / `REFUSED - <sandbox problem>` / `UNEXERCISED - <script> declares N surface(s) and produced none` (exit 1), plus two non-failing states that must stay visible: `unobserved - <script> declares ~/<surface>, which no invocation here reaches` and `unresolved - <script> declares its surface unresolved`, the second because reading `unresolved` as `none` converts an absence of evidence into a positive certification | `make verify`, through `make host-surface-observe`, which lets work through on this gate's verdict. Its OTHER consumer is the `certified=` field itself: this is the only instrument that can move a declaration from `authored` to `observed`, and a consumer reading `host-surface-check.py --manifest` weighs the two differently - that is the point of having two. Nothing downstream re-derives whether a declaration is COMPLETE; `host-surface-check.py` (row 99) checks only that one EXISTS and prints that bound in its own verdict, so an understatement is invisible to every static reading of the manifest and to the drift gate a consumer inherits from it. The `certified=observed` values are themselves re-derived here on every run - a surface claiming `observed` that a run did not observe reds - which is what keeps the field a measurement rather than a label | G, X |
| 104 | `check-behavioral-eval.py` (`controls/behavioral-eval`) | `behavioral-eval: pass` (exit 0) / `failure` (1) / `absent` (2) / `unreadable` (3) / `forged` (4) / `inconclusive` (5); usage 64, deliberately not argparse's 2, which is `absent`. Exactly one verdict maps to 0, per the rule `gate-lib.sh` states - restated and TESTED here because gate-lib enforces it only for shell gates and this one is Python. `forged` is a record whose declared `status` does not follow from its own `criteria`; it is well-formed and parses, so only re-derivation catches it. A non-boolean `mandatory`, a foreign `kind` and an unhashable enum value are refused as `unreadable` - each previously produced a confident wrong answer (a clean `pass`, a `pass` about another population, and an uncaught TypeError exiting 1 as `failure`) rather than noise. | A human reading `make verify` and the CI step output. Nothing re-derives it. | gate |
| 105 | `flow-helpers-install.sh` (`controls/flow-helpers-install`) | `FLOW_HELPERS: ok|installed|unverifiable|unverifiable-source|tampered|missing|stale|error`, exit 0/0/0/6/5/1/1/2. `tampered` and `unverifiable-source` are deliberately separate words AND separate exit codes: the first says the source DISAGREES with its recorded digest, the second says this run COULD NOT ASK - a missing digest tool, an unreadable file, a bundle carrying no manifest. Collapsing them is the failure #1185 is about, and the implementation collapsed them once during review (a failing digest tool reported `tampered` over an INTACT bundle) before `controls/flow-helpers-install/cases/bad-digest-tool-fails` pinned the distinction | the decision to INSTALL, which nothing downstream re-derives. `--check` is read by /flow:doctor and /cpp:update. BOUNDED: the manifest ships inside the bundle it certifies, so it establishes only that these bytes are the bytes recorded when the bundle was generated - it cannot detect a coherently regenerated bundle and says nothing about ARRIVAL, which needs a key outside the bundle | G |
| 106 | `step3-record-guard.sh` (`controls/step3-record-guard`) | `step3-record-guard: REFUSED - <reason>` on stdout with exit 2, or silence with exit 0. A PreToolUse hook: in a flow worktree whose branch is `issue-<N>-<slug>`, an edit is refused unless `docs/flow-runs/issue-<N>.md` exists AND records `Approval: granted`. Exactly one exemption - the record's own path, keyed on the ISSUE NUMBER and not the directory - because the record is written BY an edit and a guard without it deadlocks every run on its first edit | the EDIT ITSELF, which the harness blocks on a non-zero exit; nothing downstream re-derives it. BOUNDED, and the bound is wider than the instrument: it establishes an ARTIFACT is present, never that an APPROVAL occurred, and only on the tool paths it is matched against. It does NOT enforce #775 - meeting that needs /flow:auto to record Step 3 where a hook can read it before the first edit, which is a command-document change and the named residual. In THIS fleet coverage is near zero because sessions are steered to edit through Bash heredocs rather than Write/Edit, and that blindness is committed as a CASE rather than asserted so a change in the steer or the matcher flips it; where CPP ships it, coverage is genuinely useful | G |
| 107 | `maintain-loop-indicator.py` (`controls/maintain-loop-indicator`) | `maintain-loop: median <d>d to file, over N of M finding(s) (P% attributable, X excluded as dismissed, Y UNDETERMINED)` (exit 0), or `maintain-loop: population is EMPTY` (exit 0, an OBSERVED zero), or `maintain-loop: UNKNOWN - <reason>` (exit 2). THE PARTITION IS THE METRIC, not a number with a denominator attached: the median is computed over the attributable subset only, which on the real population is a minority, and a median presented as the headline would claim more than its input supports. UNDETERMINED is never folded into the denominator and never renders as zero. Three input states are separately registered because #1085 turns on them - an OBSERVED empty population (lists present and explicitly `[]`) reports zero and exits 0, while a ZERO-BYTE capture (what a failed `gh ... > capture.json` leaves), unparseable JSON, and a document that never declares its population are each UNKNOWN. The friction ledger is in the POPULATION but UNDETERMINED BY CONSTRUCTION: measured across every record, its keys carry no filing linkage, and matching on signal text would be inference dressed as measurement. NO BAND is proposed or implied; #1085 defers thresholds and ADR 0009 governs the first one. The control declares NO `detect_signal` - a reporter has no finding to mark, so the marker requirement is carried by `unknown_signal` (#1085 harness follow-up). Anchor is CONSTRUCTED (`constructed-inbox-without-indicator.py`) because the instrument is new: it reproduces "CPP has the inbox and not the indicator" by counting the population and computing no delay. Its most instructive case is `unknown-undeclared-population`, where it exits 0 with `0 finding(s) recorded` over a document declaring no population - an unread input rendered as a confident zero, which is why that case carries `anchor_expect: clean` | a person reading the maintain-loop baseline; nothing downstream re-derives it | G |
| 108 | `flow-plan-record.py` | `FLOW_PLAN_RECORD: present|absent|unverified` from `head-check` (exit 0/1/4), and the reported verdicts `ISSUE_DRIFT: clean|drift|unresolved` (0/3/4), `PLAN_COMPLIANCE: agreement|divergence|unknown` and `PLAN_RECORD_STABILITY: unchanged|CHANGED|unknown` (0/3/4). Every cannot-answer state has its own word and its own exit code and is never rendered as clean, agreement or present. These were six programs pasted into `flow/auto.md` (#1080, #1081, #1082) until #1211 moved them into one helper; the committed known-bad and cannot-answer cases moved with them and now run the helper itself: `tests/test_flow_plan_record.py` (a record absent at the PR head, an unreadable head, a neighbouring `issue-42Xmd`), `tests/test_flow_issue_snapshot.py` and `tests/test_flow_plan_compliance.py` (including the #1220 two-untracked-files case), each with a sweep requiring a distinctive case per unknown branch. No `controls/` registration, like rows 3-5: the cases live in `make test` | `head-check` lets the Step-6 run proceed to the merge; nothing downstream re-derives that the plan record shipped, because the Step-7 squash flattens whatever the PR carried. `drift` and `compliance` REPORT and never block - read by the run and the PR reviewer | G |
