...
- `scripts/` - Shell utilities, one per sub-bullet (add new scripts as their own line, #501):
  ...
  - install-drift - the check for the gap between the command text a session EXECUTES and the one this repo maintains (#622). `/plugin install <family>@cpp` snapshots the commands into `~/.claude/plugins/` and that snapshot moves only on re-install, while the checkout moves on every pull - so nothing noticed a box sitting 15 commits / 7 days behind, and flow:auto #65 re-diagnosed a bug #595 had already fixed and nearly filed a duplicate issue for it. Compares the marketplace clone by commit distance (resolved in the CHECKOUT, which has both ends of the range; the clone does not), the version-stamped plugin cache by per-file content parity (no git there at all, and it is the copy sessions execute), and `~/.claude/scripts/*.sh` so the SPLIT case - helpers current, markdown stale, which presents as a helper bug rather than as staleness - is named rather than inferred. `--quiet` is the hook shape (one line on drift, always exit 0), `--json`/`--list` for tooling; `skipped` is a first-class non-failing answer for a marketplace-only or checkout-only host. `make install-drift-check`, wired into the SessionStart reminder and `/cpp:update` Step 7.10. Deliberately not in `make verify`: it inspects HOME, and a CI container legitimately has no install
  ...
  - cpp-commands-link - user-scope command-surface symlinker (#663, the canonical Tier 1): per-family links `~/.claude/commands/<family>` -> the checkout's `.claude/commands/<family>`, so the executed command text follows `git pull` atomically with no cache to reconcile (the property the /plugin lane lacked, #662); touches ONLY symlinks whose target ends `/.claude/commands/<family>` (owned shape) - a user's real file/dir or foreign symlink is reported and never modified; prunes owned links for families the checkout no longer ships; `--check` read-only (foreign is reported but is NOT drift); contract `CPP_COMMANDS_LINK: ok|installed|drift|error`; run by `/cpp:init` Tier 1 and verified by `/cpp:update` Step 5c
...
### Installation
The tiered `/cpp:init` install is CANONICAL again (issue #663, reversing ADR 0001 Phase B's `/plugin` cutover - retirement of the marketplace lane is tracked in #662): Tier 1 symlinks the command surface (user scope via `scripts/cpp-commands-link.sh`, per-family `~/.claude/commands/<family>` links; optionally project scope `.claude/commands`), so the executed command text follows `git pull` atomically - `/cpp:update` is the ONE update verb and there is no package cache to reconcile. The `/plugin` marketplace lane (`/plugin marketplace add cooneycw/claude-power-pack` + `/plugin install <family>@cpp`) is DEPRECATED pending #662: its cache only refreshes on a version-stamp change that CPP's release style never produces, `/plugin update` no-ops on it, and `/plugin install` refuses when installed - the only reconciliation is per-family uninstall+reinstall (the 2026-08-11 incident). Hosts still carrying the cached families should uninstall them once the symlink tier is in place. `/cpp:init` / `/cpp:update` also install and refresh the non-command infra as before (external MCP server pointer, `@playwright/mcp` registration, secrets/AWS-SM access, spec-kit CLI, the PermissionRequest census hook, and the `/flow:*` allowlist merge). See `README.md` and `docs/HOST_MANAGED_ARTIFACTS.md`.
...
- **SessionStart pending-retro reminder** (`scripts/hook-pending-retro.sh`) is an OPT-IN, fail-open, read-only reminder: when registered, it prints ONE advisory line at session open counting pending `.claude/friction.jsonl` signals (actionable vs the bulk permission-prompt census, separately) plus uncodified `Status: proposed` learnings, and points at `/self-improvement:retro`. It only SURFACES - never codifies, never blocks - and is silent when nothing is pending. Default OFF: deliberately NOT shipped in `.claude/hooks.json` (which `/cpp:init` copies into projects), so it never turns itself on; registered user-level in `~/.claude/settings.json` only on explicit opt-in (default N) by `/cpp:init` and `/cpp:update` Step 7.7 (issue #530).
  - **Second advisory: installed-command drift** (#622) - the same hook also runs `scripts/install-drift.sh --quiet` and prints one line when the `~/.claude/plugins/` snapshot a session executes has fallen behind the checkout. The two advisories are INDEPENDENT: gating the drift line on pending retro material would hide it precisely on the clean boxes most likely to be running week-old commands. Fail-open past a missing, failing, or slow (20s `timeout`) check, and suppressible on its own with `CPP_HOOK_SKIP_INSTALL_DRIFT=1`. It inherits the hook's opt-in status, which bounds the session-open warning to opted-in hosts; everyone else gets the same answer on demand from `make install-drift-check` or `/cpp:update` Step 7.10.
