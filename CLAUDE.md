...
### Installation
CPP's own marketplace lane is retired (#662, ADR 0005). Commands are used from the checkout; issue #663 restores the tiered `/cpp:init` + `/cpp:update` symlink surface as canonical. Until that migration lands, uninstall each lingering family with `/plugin uninstall <family>@cpp`. Claude Code's native `/plugin` remains available for third-party content. See `README.md` and `docs/HOST_MANAGED_ARTIFACTS.md`.

- `/cpp:init` - Host-infra installer today (Tiers: Minimal, Standard, Full, CI/CD, Codex); #663 restores its tiered symlink command installation as the canonical surface. It wires the external second-opinion `.mcp.json` pointer, `@playwright/mcp`, the census hook + flow allowlist, optionally spec-kit, and - at the Codex tier - the generated Codex skills plus common-memory harness. Init only ever INSTALLS; retired-surface teardown belongs to `/cpp:update`
