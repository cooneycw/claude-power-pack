...
- `scripts/` - Shell utilities, one per sub-bullet (add new scripts as their own line, #501):
  ...
  - sandbox-phase1-trial - ADR 0002 Phase 1 empirical trial harness (#548): runs the E1-E6 sandbox exit-bar checks in a throwaway, project-scoped trial (nested `claude -p` write-containment probes + a `bwrap` ro-bind primitive); never touches `~/.claude/settings.json`
