...
- `codex/skills/` - Codex SKILL.md skills, the second harness surface (issue #555, companion to codex-power-pack epic cooneycw/codex-power-pack#64): generated `<family>-<cmd>/` skill dirs emitted from the same `.claude/commands/<family>/` single source by `scripts/codex-skill-sync.py`; the cross-repo publish into codex-power-pack is issue #556. Regenerate with `make codex-skills` after any command or referenced-script edit; `make codex-init` installs to `~/.codex/skills/`. Hand-curated skill dirs (no GENERATED marker) are never touched.
- `codex/prompts/` - Codex CLI custom prompts (issue #446): generated `<family>-<command>.md` files (Codex `/<family>-<command>`) emitted from the same `.claude/commands/<family>/` single source by `scripts/codex-prompt-sync.py`, plus the hand-curated `cpp-memory.md` (#433, never overwritten). DEPRECATED flat surface (#555): superseded by `codex/skills/`, retired at the #556 cutover; until then regenerate with `make codex-prompts` after any command edit.
