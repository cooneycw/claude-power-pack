...
| Flow advisory helpers | `~/.claude/scripts/{check-ignored-additions, friction-log}.sh` | the two always-on advisory helpers `/flow:auto` runs bare at the stable path but that #581 never allowlisted (issue #616): `friction-log.sh` appends a friction record to `.claude/friction.jsonl` on every capture point of every flow run, `check-ignored-additions.sh` reads git state to warn when a blanket ignore rule swallowed a file meant to be tracked (Step 6, `/flow:finish`). Both are bundled in the flow plugin (EXTRA_FILES, #590) and installed by `/flow:repair`, i.e. already part of the audited family - read-only-or-append, consistent with the one-reviewed-script-per-rule policy |
...
| Command-surface symlinker | `~/.claude/scripts/cpp-commands-link.sh` | the canonical Tier 1 command surface (issue #663): writes ONLY per-family symlinks under `~/.claude/commands/` whose target shape it owns (`/.claude/commands/<family>`), never a user file/dir; `--check` is read-only; run by `/cpp:init` Tier 1 and `/cpp:update` Step 5c |
